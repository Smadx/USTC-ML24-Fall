...
def valueIteration(
    ...
    def computePolicy(V: Dict[StateT, float]) -> Dict[StateT, ActionT]:
        ...
        for state, actions in stateActions.items():
            pi[state] = max(actions, key=lambda action: computeQ(V, state, action))
        return pi
        # END_YOUR_CODE
        ...
        if self.numIters % self.calcValIterEvery == 0:
            ...
            for (state, action), nextStates in self.tCounts.items():
                for nextState, count in nextStates.items():
                    ...
                    succAndRewardProb[(state, action)].append((nextState, transitionProb, expectedReward))
...
class TabularQLearning(RLAlgorithm):
    ...
    def incorporateFeedback(
        ...
        target = reward
        if not terminal:
            target += self.discount * max([self.Q[nextState, nextAction] for nextAction in self.actions])
